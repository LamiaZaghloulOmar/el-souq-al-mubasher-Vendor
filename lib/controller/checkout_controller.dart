...
class CheckOut extends GetxController implements GetxService {
  final OrderRepo orderRepo;
  CheckOut({@required this.orderRepo});
  ...
  Future<List<OrderDetailsModel>> getOrderDetails(String orderID) async {
    ...
    Response response = await orderRepo.getOrderDetails(int.parse(orderID));
    _isLoading = false;
    if (response.statusCode == 200) {
      _orderDetails = [];
