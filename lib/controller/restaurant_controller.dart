...
class RestaurantController extends GetxController implements GetxService {
    ...
    if (product != null && product.addOns != null) {
      for (int index = 0; index < product.addOns.length; index++) {
        setSelectedAddonIndex(
            _addonsIds.indexOf(product.addOns[index].id), false);
      }
    }
  ...
  Future<void> updateRestaurant(Restaurant restaurant, String token) async {
    ...
    Response response = await restaurantRepo.updateRestaurant(
        restaurant, _pickedLogo, _pickedCover, token);
    if (response.statusCode == 200) {
      Get.back();
      ...
      for (int index = 0; index < _variantTypeList.length; index++) {
        _fields.addAll(<String, String>{
          'price_${_variantTypeList[index].variantType.replaceAll(' ', '_')}':
              _variantTypeList[index].controller.text
        });
      }
    }
    Response response =
        await restaurantRepo.addProduct(product, _pickedLogo, _fields, isAdd);
    if (response.statusCode == 200) {
  ...
  Future<void> getRestaurantReviewList(String restaurantID) async {
    ...
    Response response =
        await restaurantRepo.getRestaurantReviewList(int.parse(restaurantID));
    if (response.statusCode == 200) {
...
*/
  ...
  void setCategoryIndex(
    ...
    getRestaurantProductList(_categoryIds[_categoryIndex], 1,
        Get.find<RestaurantController>().type, false);
    update();
  }
